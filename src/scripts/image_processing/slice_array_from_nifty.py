...
def get_l3_seg_array_by_id(patient_id,l3_slice_manual,seg_dir):
    
    seg_path = get_image_path_by_id(patient_id,seg_dir)
    seg_sitk =  sitk.ReadImage(seg_path)
    seg_array  = sitk.GetArrayFromImage(seg_sitk)[l3_slice_manual,:,:]
...
def get_l3_image_array_by_id(patient_id,l3_slice_manual,image_dir):
    
    image_path = get_image_path_by_id(patient_id,image_dir)
    image_sitk =  sitk.ReadImage(image_path)
    image_array = sitk.GetArrayFromImage(image_sitk)[l3_slice_manual,:,:]
