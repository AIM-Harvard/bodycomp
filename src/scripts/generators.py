    ...
    def __getitem__(self, idx):
        ...
        if self.jitter:
            ...
            for i in batch_inds:
                ...
                stacked = np.dstack([self.images[i, :, :, :].astype(float), self.masks[i, :, :, :]])
...
class SliceSelectionSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in batch_inds:
            ...
            # Load in image
            image_order = str(i).zfill(6)
            file_name = [os.path.relpath(os.path.join(self.image_dir, x)) \
                            for x in os.listdir(self.image_dir) \
                            if os.path.isfile(os.path.join(self.image_dir, x)) and image_order in x][0]
